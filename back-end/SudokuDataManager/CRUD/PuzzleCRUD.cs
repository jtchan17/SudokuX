...
namespace SudokuDataManager
{
    public class PuzzleCRUD
    {
        ...
        {
            string diff = InputHelper.ReadString("Difficulty (Easy/Medium/Hard): ");
            string grid = InputHelper.ReadString("Enter puzzle grid (81 chars): ");
        ...
        {
            return Puzzles
                    .Where(p => p is SudokuPuzzle)
                    .Cast<SudokuPuzzle>()
                    .FirstOrDefault(p => p.PuzzleId == id);
        }

        public static void DeletePuzzle()
        {
            ViewPuzzles();
            int id = InputHelper.ReadInt("Enter Puzzle ID: ");
